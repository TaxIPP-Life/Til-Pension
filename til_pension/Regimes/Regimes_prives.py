...
from til_pension.trimesters_functions import trim_ass_by_year, validation_trimestre, imput_sali_avpf, trim_mda
...
class RegimeGeneral(RegimePrive):
    ...
    def sal_regime(self, trim_cot_by_year_regime, trim_cot_by_year_avpf, FP_to_RG):
        return trim_cot_by_year_regime[1] + trim_cot_by_year_avpf[1] + FP_to_RG[1]

    def trim_maj_mda_ini(self, data, trim_cot_by_year_regime, trim_ass_by_year):
        P_mda = self.P.prive.RG.mda
        info_ind = data.info_ind
        return trim_mda(info_ind, self.name, P_mda)*(trim_cot_by_year_regime[0].sum(axis=1) + trim_ass_by_year.sum(axis=1) > 0)
...
#         return trim_ass_by_year(data, self.code_regime, compare_destinie)
    ...
    def trim_maj_mda_ini(self, data, trim_cot_by_year_regime):
        P_mda = self.P.prive.RG.mda
        info_ind = data.info_ind
        return trim_mda(info_ind, self.name, P_mda)*(trim_cot_by_year_regime[0].sum(axis=1) > 0)
