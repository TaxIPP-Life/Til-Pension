...
class Generation(object):
    '''
    Object qui contient des valeurs d'une variable par génération
    '''
    def __init__(self, name = 'untitled Generation', option = None, unit = None, control = None):
        self._name = name
        self._tranches = []
        self._nb = 0
        self._tranchesM = []
    ...
    def multValeur(self, factor, inplace = True, new_name = None):
        if inplace:
            for i in range(self._nb):
                self.setValeur(i, factor * self.valeur[i])
        else:
            if new_name is None:
                new_name = self._name
            b = Generation(new_name, option = self._option, unit = self.unit)
            for i in range(self.nb):
                b.addTranche(self.seuils[i], self.valeur[i])
            b.multValeur(factor, inplace = True)
            return b
    ...
    def addTranche(self, seuil, valeur):
        if seuil in self.seuils:
            i = self.seuils.index(seuil)
            self.setTaux(i, self.taux[i] + valeur)
        else:
            self._tranches.append([seuil, valeur])
            self._tranches.sort()
            self._nb = len(self._tranches)
    ...
    def addTrancheM(self, seuil, valeur):
        if seuil in self.seuilsM:
            i = self.seuilsM.index(seuil)
    ...
    def __iter__(self):
        return itertools.izip(self.seuils, self.valeur)

    def __str__(self):
        output = self._name + '\n'
        for i in range(self._nb):
            output += str(self.seuils[i]) + '  ' + str(self.valeur[i]) + '\n'
        return output
