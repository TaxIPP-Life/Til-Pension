...
def flow_contributions_mat(pension_contrib, nominal=True, vector = True):
    ...
    year_min = int(flow_contrib.columns[0]) // 100
    year_max = int(flow_contrib.columns[-1]) // 100
...
def flow_fct_builder(table, nominal=True, jac_inverse = False):
    ''' These functions creates the vectorial function taking as an argument the vector r (1*N) of TRI '''
    pension_contrib = table.copy()
    del table
    dates_contrib = [col for col in pension_contrib.columns if str(col)[0:2] in ['19', '20']]
    pension_contrib.loc[:, dates_contrib] = - pension_contrib.loc[:, dates_contrib]
    pension_contrib = pension_contrib.apply(add_pension, axis = 1)
    ...
    year_min = int(pension_contrib.columns[0]) // 100
    year_max = int(pension_contrib.columns[-1]) // 100
...
def indices_prix(year_start, year_end, year_ref = 2009, path_xlsx = path_xlsx, return_dict = False):
...
def add_pension(row):
    ...
    for date in dates_pension:
        row[date] = row['pension']
    return row


def tri(pensions_contrib, nominal = False, high_dimension = True, marginal=False):
    print "   Initial functions to calculate TRI have been built"
    initial_value = 0.98 * np.ones(pensions_contrib.shape[0])
    if high_dimension:
        fct, jac_diag = flow_fct_builder(pensions_contrib, nominal = nominal,
                                         jac_inverse = False)
        # alpha = -1 / np.median(jac_diag(initial_value))
        # sol = root(fct, initial_value, method = 'excitingmixing',
        # options = {'maxiter': 400, 'jac_options': {'alpha': alpha}})['x']
...
def tri_margin(delta_contrib, delta_pension, age_delta):
